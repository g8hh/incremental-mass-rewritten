	...
	*/
	...
	for (let x in BEYOND_RANKS.rewards) {
		...
		for (let y in BEYOND_RANKS.rewards[x]) {
			table += `<span id="br_reward_${x}_${y}"><b>${getRankTierName(x+5)}${format(y,0)}:</b>${BEYOND_RANKS.rewards[x][y]}${ee&&BEYOND_RANKS.rewardEff[x][y]?`目前效果：<span id='br_eff_${x}_${y}'></span>`:""}</span><br>`
		}
		table += '<br>'
	}
...
function updateTickspeedHTML() {
	...
	if (unl) {
		...
		else tmp.el.tickspeed_eff.setTxt(format(teff.eff)+"倍")
...
function updateBlackHoleHTML() {
	...
	*/
	...
	if (unl) {
		...
		tmp.el.bhUnstableEffect.setHTML(format(tmp.unstable_bh.effect)+"次方")
