        ...
        photon: [
            {
                ...
                effDesc(x) { return hasElement(204)?format(x)+"次方":format(x)+"倍" },
            },{
                ...
                effDesc(x) { return hasElement(204)?format(x)+"次方":format(x)+"倍"+(x.gte(1e15)?"<span class='soft'>(softcapped)</span>":"") },
            },{
                ...
                effDesc(x) { return hasElement(213)?"^"+format(x):format(x)+"倍" },
            },
        ],
        gluon: [
            {
...
function updateBosonsHTML() {
    ...
    for (let x in BOSONS.names) {
        ...
        if (tmp.bosons.effect[id]) for (let y in tmp.bosons.effect[id]) {
            if (y == '2' && id == 'pos_w') tmp.el[id+"_eff"+y].setHTML(hasElement(250) ? '，<br>并使质量获取速度变为原来的' + format(tmp.bosons.effect[id][y]) + '次方' : '')
