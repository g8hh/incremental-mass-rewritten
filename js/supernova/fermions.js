        ...
        [
            {
                ...
                desc(x) {
                    return `使Z<sup>0</sup> Boson's first effect is ${format(x.sub(1).mul(100))}% stronger`+(x.gte(5)?"<span class='soft'>(softcapped)</span>":"")
                },
                inc: "Mass",
    ...
    for (i = 0; i < 2; i++) {
        ...
        for (let x = 0; x < FERMIONS.types[i].length; x++) {
            ...
            if (unl) {
                ...
                tmp.el[id+"_tier"].setTxt(format(player.supernova.fermions.tiers[i][x],0)+(Decimal.lt(tmp.fermions.maxTier[i][x],EINF)?"，上限为"+format(tmp.fermions.maxTier[i][x],0):"") + (tmp.fermions.bonuses[i][x].gt(0)?"，额外阶层为"+tmp.fermions.bonuses[i][x].format():""))
