        ...
        [
            {
                ...
                effectDesc: x => "质量变为原来的"+x[0].format(0)+"次方"+x[0].softcapHTML(1e3)+"，黑洞质量变为原来的"+x[1].format(0)+"次方",
            },{
        ...
        if (tmp.stab[8] == 0) updateCoreHTML()
            ...
            for (let t in CORE) {
                ...
                for (let i = 0; i < MAX_STARS; i++) {
                    if (s[i].gt(0)) hh += "综合评分"+format(s[i],2)+" | <i></i>"+(ct.preEff[i] || '???.')+`<b class='sky'>(${ct.effDesc[i](ctmp[i])})</b><br>`
                }
            ...
            for (let r in INF.upgs) {
                ...
                for (let c in ru) {
                    ...
                    if (el) {
                        ...
                        tmp.el[`iu_${id}_desc`].setHTML(b?u.effectDesc?"<br>效果："+u.effectDesc(infUpgEffect(id)):"":"<br>Cost: <b>"+u.cost.format(0)+"</b> Infinity Points")
                    }
                }
            }
        }
