const PRIM = {
    ...
    particle: {
        ...
        effDesc: [
            ...
            x=>{ return `使狂怒能量获取速度变为原来的${format(x[0]) + x[0].softcapHTML(3,hasUpgrade('br',16))}次方/<br> Boost Non-Bonus Tickspeed by ${format(x[1])}x` },
            x=>{ return `使暗物质获取速度变为原来的${format(x[0]) + x[0].softcapHTML(3,hasUpgrade('br',16))}次方/<br> Boost BH Condenser Power by ${format(x[1])}x` },
            x=>{ return `使原子获取速度变为原来的${format(x[0]) + x[0].softcapHTML(3,hasUpgrade('br',16))}次方/<br> Boost Cosmic Ray Power by ${format(x[1])}x` },
