        ...
        ids: [
            {
                ...
                effDesc(x) { return format(x,0)+"倍"+(x.gte('e1.2e4')?`<span class='soft'>(softcapped${x.gte('e2e400')?"^2":""})</span>`:"")},
            },{
                ...
                effDesc(x) { return "强化"+(x.gte(10)?format(x)+"倍":format(x.sub(1).mul(100))+"%") },
            },{
...
function updateMDHTML() {
    ...
    tmp.el.md_eff.setTxt(tmp.md.mass_eff.gte(10)?format(tmp.md.mass_eff)+"倍":format(tmp.md.mass_eff.sub(1).mul(100))+"%")
    ...
    for (let x = 0; x < MASS_DILATION.upgs.ids.length; x++) {
        ...
        if (unl) {
            ...
            tmp.el["md_upg"+x+"_lvl"].setTxt(format(player.md.upgs[x],0)+(upg.maxLvl!==undefined?"，上限为"+format(upg.maxLvl,0):""))
...
function updateBDHTML() {
    ...
    for (let x = 0; x < MASS_DILATION.break.upgs.ids.length; x++) {
        ...
        if (unl) {
            ...
            tmp.el["bd_upg"+x+"_lvl"].setTxt(format(bd.upgs[x],0)+(upg.maxLvl!==undefined?"，上限为"+format(upg.maxLvl,0):""))
