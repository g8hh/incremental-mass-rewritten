        ...
        ids: [
            {
                ...
                effDesc(x) { return "增加"+format(x.sub(1).mul(100))+"%" },
            },
        ],
    },
...
function updateMDHTML() {
    ...
    for (let x = 0; x < MASS_DILATION.upgs.ids.length; x++) {
        ...
        if (unl) {
            ...
            tmp.el["md_upg"+x+"_cost"].setTxt(player.md.upgs[x].lt(upg.maxLvl||EINF)?"花费："+formatMass(tmp.md.upgs[x].cost):"")
        }
    }
...
function updateBDHTML() {
    ...
    for (let x = 0; x < MASS_DILATION.break.upgs.ids.length; x++) {
        ...
        if (unl) {
            ...
            tmp.el["bd_upg"+x+"_cost"].setTxt(bd.upgs[x].lt(upg.maxLvl||EINF)?"花费："+formatMass(tmp.bd.upgs[x].cost):"")
        }
    }
}
