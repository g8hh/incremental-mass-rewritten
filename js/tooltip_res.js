const TOOLTIP_RES = {
    ...
    quarks: {
        ...
        desc() {
            ...
            h += `<br>(<b>溢出</b>之前为<b>${format(tmp.overflowBefore.quark,0)}</b>)`;
    ...
    md: {
        ...
        desc() {
            ...
            h += `<br>(<b>溢出</b>之前为<b>${formatMass(tmp.overflowBefore.dm)}</b>)`;
    ...
    sn: {
        ...
        desc() {
            let h = `
            You have becomed <i></i>${getScalingName('supernova')}<b>${player.supernova.times.format(0)}</b>次超新星。
            <br class='line'>
...
function updateTooltipResHTML(start=false) {
    for (let id in TOOLTIP_RES) {
        ...
        if (tr) tr.setTooltip(`<h3>[ <i></i>${tr_data.full}<i></i> ]</h3>`+(tr_data.desc?"<br class='line'>"+tr_data.desc():""))
    }
}