...
function updateElementsHTML() {
    ...
    tmp.el.elemLayer.setHTML("元素阶层-"+["普通","μ子"][elayer])
    ...
    if (ch) {
        ...
        let res = [eu.inf?"无限点数":eu.dark?"黑暗之影":"夸克",eu.cs?"腐化星辰":"奇异原子"][elayer]
        ...
        tmp.el.elem_cost.setTxt(format(eu.cost,0)+res+(eu.c16?"(需进入挑战16)":!infU7&&BR_ELEM.includes(ch)?"(需进入大撕裂)":"")+(player.qu.rip.active&&tElem.cannot.includes(ch)?"[大撕裂中无法购买]":""))
