const BUILDINGS_DATA = {
    mass_1: {
        ...
        get_effect: x => "质量获取速度+"+formatMass(x.effect)+"",
    },
    mass_2: {
        name: "助推器",
		icon: "mass_upg2",
        ...
        get_power: x => "+"+format(x.power)+"倍",
        get_effect: x => "锻体器倍率变为"+format(x.effect)+"倍",
    },
    mass_3: {
        name: "强化器",
		icon: "mass_upg3",
        ...
        get_power: x => "+"+format(x.power)+"次方",
        get_effect: x => "助推器倍率变为"+format(x.effect)+"次方"+(x.effect.gte(x.ss)?`<span class='soft'>(softcapped${x.effect.gte(1.8e5)?x.effect.gte(5e15)&&!player.ranks.pent.gte(15)?"^3":"^2":""})</span>`:""),
    },
    mass_4: {
        name: "降伏器",
		icon: "mass_upg4",
        ...
        get_power: x => "+"+format(x.power)+"次方",
        get_effect: x => "强化器倍率变为"+format(x.effect)+"次方"+x.effect.softcapHTML(x.ss),
    },
    tickspeed: {
        name: "时间速度",
		icon: "tickspeed",
        ...
        get_power: x => (x.power.gte(10)?format(x.power)+"倍":formatPercent(x.power.sub(1)))
		+(x.power.gte(x.ss)&&!hasUpgrade('rp',16)?"<span class='soft'>(softcapped)</span>":""),
        get_effect: x => "质量获取速度变为" + format(x.effect) + (hasElement(199) && !CHALS.inChal(15) ? "次方" : "倍"),
    },
    accelerator: {
        name: "加速器",
		icon: "accelerator",
        ...
        get_power: x => "+"+format(x.power)+"次方",
        get_effect: x => "时间速度效果变为"+format(x.effect)+"次方"+x.effect.softcapHTML(x.ss),
    },
    bhc: {
        name: "黑洞压缩器",
		icon: "bhCondenser",
        ...
        get_power: x => format(x.power)+"倍",
        get_effect: x => "黑洞质量获取速度变为"+format(x.effect)+"倍",
    },
    fvm: {
        name: "假真空操控器",
		icon: "fvm",
        ...
        get_power: x => format(x.power)+"倍",
        get_effect: x => "不稳定黑洞的获取速度变为"+format(x.effect)+"倍",
    },
    cosmic_ray: {
        name: "宇宙射线",
		icon: "gamma_ray",
        ...
        get_power: x => format(x.power)+"倍",
        get_effect: x => format(x.effect)+"倍"+(hasGlyphUpg(12)?"，"+format(x.exp)+"次方":""),
    },
    star_booster: {
        name: "星辰助推器",
		icon: "star_booster",
        ...
        get_power: x => format(x.power) + "倍" + x.power.softcapHTML(1e13,hasUpgrade('atom',22)),
        get_effect: x => "星辰发生器变为" + format(x.effect) + "倍" + x.effect.softcapHTML('e3e18',hasUpgrade('atom',22)),
    },
    cosmic_string: {
        name: "宇宙弦",
		icon: "cosmic_string",
        ...
        get_power: x => format(x.power) + "倍",
        get_effect: x => "蓝图粒子获取速度变为" + format(x.effect) + "倍",
    },
    pe: {
        name: "平行挤出机",
		icon: "pe",
        ...
        get_power: x => format(x.power)+ "倍",
        get_effect: x => "维度质量获取速度变为" + format(x.effect) + "倍",
    },
...
const BUILDINGS = {
		...
		for (var [i, b] of Object.entries(BUILDINGS_DATA)) {
			...
			if (el.el) el.setHTML(`<div class="table_center upgrade" style="width: 100%; margin-bottom: 5px;">
				...
				<div style="margin-left: 5px; text-align: left; width: 400px">
					倍率：<span id="building_pow_${i}"></span><br>
					效果：<span id="building_eff_${i}"></span>
				</div>
			</div>`)
		}
	},
	update(i) {
        ...
        tmp.el["building_cost_"+i].setHTML(allow ? "花费："+b.get_cost(cost) : "未解锁" + b.denyPurchaseText??"")
