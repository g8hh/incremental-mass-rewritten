const ATOM = {
    ...
    reset() {
        if (tmp.atom.canReset) {
            if (player.confirms.atom) createConfirm("您确定要重置吗？",'atomReset',CONFIRMS_FUNCTION.atom)
...
function updateAtomicHTML() {
	...
	tmp.el.gamma_ray_eff.setHTML(format(tmp.atom.gamma_ray_eff.eff)+"倍"+(hasGlyphUpg(12)?"，"+format(tmp.atom.gamma_ray_eff.exp)+"次方":""))
