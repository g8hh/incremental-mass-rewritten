const RESOURCES_DIS = {
    ...
    sn: {
        ...
        desc: (gs)=>{
            ...
            let h = tmp.inf_unl?format(g.mul(FPS),0)+"/秒":format(g,0)
    ...
    inf: {
        ...
        desc: (gs)=>player.inf.points.format(0)+"<br>(+"+tmp.IP_gain.format(0)+")"+"<br>(无限进度"+formatPercent(player.mass.max(1).log10().max(1).log10().div(tmp.inf_limit.max(1).log10().max(10).log10()).max(0).min(1))+(tmp.brokenInf?")":")"),
