...
function updateMattersHTML() {
    ...
    for (let i = 0; i < 14; i++) {
        ...
        if (unl) {
            ...
            if (i > 0) {
                ...
                tmp.el['matter_upg_eff'+i].setHTML((c16?"×":"^")+tu.eff.format(2)+(tu.exp.gt(1) ? "，" + (c16?"":"指数变为") + tu.exp.format() + "次方" : ""))
    ...
    tmp.el.FSS_eff1.setHTML(
        ...
        ? `由于最终星辰碎片的效果，物质获取速度变为原来的${tmp.matters.FSS_eff[0].format(1)}次方`.corrupt(c16 && !hasElement(11,1))
        : ''
    )
}
