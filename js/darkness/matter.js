...
function updateMattersHTML() {
    ...
    tmp.el.FSS_eff1.setHTML(
        ...
        ? `由于最终星辰碎片的效果，物质获取速度变为原来的${tmp.matters.FSS_eff[0].format(1)}次方`
        : ''
    )
}
    ...
    for (let i = 0; i < 15; i++) {
        if (i < 14) {
            ...
            <div class="matter_div final" id="final_star_shard_div">
                ...
                <br><br>
                <button class="btn full" id="FSS_btn" onclick="MATTERS.final_star_shard.reset()">To get Final Star Shard, reset dark shadows, abyssal blots and matters, and force to Darkness reset. It boosts matters and glyphic mass.<br>Require: <span id="FSS_req">???</span> FSS base
                </button>
            </div>
            `
        }
